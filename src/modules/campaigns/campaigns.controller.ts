...
import type { CampaignCreateBody, CampaignUpdateBody } from '../../routes/campaigns.schemas.js';
...
export async function listCampaigns(request: FastifyRequest) {
...
export async function createCampaign(request: FastifyRequest, reply: FastifyReply) {
  const { workspaceId } = getJwtUser(request);
  const body = request.body as CampaignCreateBody;
...
export async function updateCampaign(request: FastifyRequest, reply: FastifyReply) {
  const { workspaceId } = getJwtUser(request);
  const { id } = request.params as { id: string };
  const body = request.body as CampaignUpdateBody;
...
export async function cancelCampaign(request: FastifyRequest, reply: FastifyReply) {
  const { workspaceId } = getJwtUser(request);
  const { id } = request.params as { id: string };
  const campaign = await prisma.campaign.findFirst({ where: { id, workspaceId } });
  if (!campaign) return reply.code(404).send({ error: 'Not found' });
...
export async function resumeCampaign(request: FastifyRequest, reply: FastifyReply) {
  const { workspaceId } = getJwtUser(request);
  const { id } = request.params as { id: string };
  const campaign = await prisma.campaign.findFirst({ where: { id, workspaceId } });
  if (!campaign) return reply.code(404).send({ error: 'Not found' });
...
export async function resendFailedCampaign(request: FastifyRequest, reply: FastifyReply) {
  const { workspaceId } = getJwtUser(request);
  const { id } = request.params as { id: string };
  try {
...
export async function resendRecipient(request: FastifyRequest, reply: FastifyReply) {
  const { workspaceId } = getJwtUser(request);
